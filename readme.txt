...
Contributors: fatoripay, aztecweb
Donate link:
Tags: fatoripay, payment, payment gateway, woocommerce, credit card, bank ticket
...
Take transparent credit card and bank ticket payment checkouts on your store using FatoriPay.

== Description ==

Use [FatoriPay](https://www.fatoripay.com) as payment method in your WooCommerce store.

This plugin is an implementation of [FatoriPay API v3](https://fatoripayv3.docs.apiary.io). The checkout mechanism is completely transparent. The customer will not go out of your store to finish the order. The data are sent to FatoriPay service, that process the payment and return its status.
...
1. Search for _FatoriPay Gateway for WooCommerce_.
1. Click in _Install Now_ and, after, in _Activate_.
1. Visit _WooCommerce > Settings > Checkout_.
1. Visit _FatoriPay Ticket_ and _FatoriPay Credit Card_ to enable, set up and integrate the gateway with FatoriPay system.

= From WordPress.org =

1. Download _FatoriPay Gateway for WooCommerce_
1. Unzip the file and upload the _fatoripay-woo_ directory to your _wp-content/plugins_ directory using your favorite method (ftp, sftp, scp, etc…). Or visit _Plugins > Add New > Upload Plugin_, select _fatoripay-woo.zip_ file, click in _Install Now_ and, after, in _Activate Plugin_.
1. Search for _FatoriPay Gateway for WooCommerce_.
1. Visit _WooCommerce > Settings > Checkout_.
1. Visit _FatoriPay Ticket_ and _FatoriPay Credit Card_ to enable, set up and integrate the gateway with FatoriPay system.
...
= 1.3.2 =
* Tweak - Allow disable overdue boleto on FatoriPay removal
* Fix - Remove only store overdue boleto on FatoriPay
...
* Tweak - Remove overdue boleto on FatoriPay automatically
...
= 1.1.0 =
* Tweak - Added `woocommerce_fatoripay_request_api_key` hook to manipulate the request api key
* Tweak - Added `woocommerce_fatoripay_set_customer_params` and `woocommerce_fatoripay_set_customer_action` hook to manipulate the customer request
...
* Tweak - Update webhook message when integrate got FatoriPay API error status code
...
= 1.0.2 =
* Tweak - Validate if webhook payment status is the same of registered in FatoriPay
