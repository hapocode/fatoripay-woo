...
namespace WC_FatoriPay\Installments\Gateway;

use Exception;
use WC_FatoriPay\Gateway\Gateway;
use WC_FatoriPay\Installments\Checkout\Installments_Checkout;
use WC_FatoriPay\Installments\Checkout\Order_Interest_Handler;
use WC_Order;
...
class Payment_Installments {
	...
	public function installment_payment_data( array $payment_data, WC_Order $wc_order, Gateway $gateway ) : array {
		...
		$installments   = absint( $posted_data[ "fatoripay_{$gateway_prefix}_installments" ] );
	...
	private function get_gateway_fields_prefix( string $gateway_id ) : string {
		if ( 'fatoripay-ticket' === $gateway_id ) {
			return 'ticket';
		}
