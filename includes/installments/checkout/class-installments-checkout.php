...
namespace WC_FatoriPay\Installments\Checkout;

use WC_FatoriPay\Api\Api_Limit;
use WC_FatoriPay\Gateway\Gateway;
use WC_FatoriPay\Installments\Admin\Settings\Default_Installment_Settings;
use WC_FatoriPay\Installments\Helper\Installments_Calculator_Helper;
use WC_FatoriPay\Installments\Helper\Values_Formater_Helper;
...
class Installments_Checkout {
	...
	public function get_installments_field( string $field_prefix ) : array {
		$field = array(
			'type'     => 'select',
			'label'    => __( 'Installments', 'fatoripay-woo' ),
			'required' => false,
			'class'    => array( 'fatoripay_field_' . $field_prefix ),
			'id'       => "fatoripay-{$field_prefix}-installments",
	...
	private function get_installment_option_with_interest( int $installment, float $value ) : string {
		...
		$option_value = sprintf(
			...
			__( '%1$dx %2$s (%3$s%% interest)', 'fatoripay-woo' ),
			$installment,
