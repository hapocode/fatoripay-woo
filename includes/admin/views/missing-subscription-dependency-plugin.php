...
$dependency = WC_FatoriPay\Admin\Plugin_Dependency::get_instance();
$plugins    = get_plugins();
...
foreach ( $dependency->get_subscription_dependencies() as $plugin_slug => $plugin ) :
	if ( is_plugin_active( $plugin['plugin_file'] ) ) :
		if ( $plugins[ $plugin['plugin_file'] ]['Version'] < $plugin['min_version'] ) {
			...
			$button_label  = __( 'Visit plugin site', 'fatoripay-woo' );
			$target        = '_blank';
		} else {
			continue;
		}
		...
		if ( ! key_exists( $plugin['plugin_file'], $plugins ) ) :
			...
			$button_label = sprintf( __( 'Install %s', 'fatoripay-woo' ), $plugin['name'] );
			...
			/* translators: %s: The plugin name  */
			$button_label = sprintf( __( 'Activate %s', 'fatoripay-woo' ), $plugin['name'] );
		endif;
	endif;
	...
	<div class="missing-subscription-dependency-plugin">
		<p>
			<?php
				...
				echo wp_kses_post( sprintf( __( 'The subscriptions feature depends on the <strong>%1$s plugin version %2$s or greater</strong> to work.', 'fatoripay-woo' ), $plugin['name'], $plugin['min_version'] ) );
			?>
		</p>
