...
namespace WC_FatoriPay\Api\Client;

use WC_FatoriPay\Gateway\Gateway;
use WC_FatoriPay\Api\Response\Response_Factory;
use WC_FatoriPay\Api\Response\Response;
...
class Client {
	...
	/**
	 * Force body data format for all FatoriPay GET requests
	 *
	 ...
	 * @return Response The FatoriPay API response object.
	 */
	protected function request( $method, $endpoint, $data = array(), $filter_callback = false ) {
		...
		$response = $this->http->request(
			$url, array(
				...
				'headers' => array(
					'access_token' => apply_filters( 'woocommerce_fatoripay_request_api_key', $this->gateway->get_api_key(), $data ),
