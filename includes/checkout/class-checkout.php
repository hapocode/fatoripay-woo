...
namespace WC_FatoriPay\Checkout;
...
class Checkout {
	...
	public function handle_woocommerce_subscriptions_checkout_usage( $order ) {
		...
		if ( $payment_gateway ) {
			if ( in_array( $payment_gateway->id, array( 'fatoripay-credit-card', 'fatoripay-ticket', 'fatoripay-pix' ), true ) ) {
				$payment_id = $payment_gateway->get_payment_id_from_order( $order );
				if ( false !== $payment_id ) {
					// If we are only processing payment for an existing fatoripay order,
