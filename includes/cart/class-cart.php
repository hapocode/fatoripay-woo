...
class Cart {
	...
	/**
	 * Disables FatoriPay gateways
	 *
	 * @param array $available_gateways Available payment gateways.
	 * @return array List of payment gateways without FatoriPay
	 */
	public function disable_fatoripay_gateways( $available_gateways ) {
		unset( $available_gateways['fatoripay-credit-card'], $available_gateways['fatoripay-pix'], $available_gateways['fatoripay-ticket'] );
		return $available_gateways;
	}

	/**
	 * Shows the notice with the reason to disable FatoriPay gateways
	 *
	...
	public function check_available_payment_gateways( $available_gateways ) {
		...
		if ( ! is_admin() ) {
			...
			if ( false !== $order_id ) {
					...
					foreach ( WC()->cart->get_cart() as $key => $value ) {
						...
						if ( in_array( $product->get_type(), $subscriptions_helper->subscription_product_types, true ) ) {
							...
							if ( false === $subscriptions_helper->convert_period( $product->get_meta( '_subscription_period_interval' ), $product->get_meta( '_subscription_period' ) ) ) {
								$available_gateways = $this->disable_fatoripay_gateways( $available_gateways );

								$message = __( 'The FatoriPay payment gateway was disabled because it does not support the billing cycle for one or more products in the cart.', 'fatoripay-woo' );
								$this->show_notice_disabled_gateway_reason( 'unsupported-fatoripay-billing-cycle', $message );
				...
				if ( true === $cart_has_subscription_products && count( WC()->cart->get_applied_coupons() ) > 0 ) {
					foreach ( WC()->cart->get_applied_coupons() as $coupon_code ) {
						...
						if ( false === $subscriptions_helper->discount_coupon_supported( $coupon ) ) {
							...
							$message = sprintf( __( 'The FatoriPay payment gateway was disabled because it does not support the following coupon code: %s.', 'fatoripay-woo' ), $coupon_code );
							$this->show_notice_disabled_gateway_reason( 'unsupported-fatoripay-discount-coupon', $message );
				...
				// Disables PIX.
				if ( true === $cart_has_subscription_products && array_key_exists( 'fatoripay-pix', $available_gateways ) ) {
					unset( $available_gateways['fatoripay-pix'] );
				}
			}
		}
