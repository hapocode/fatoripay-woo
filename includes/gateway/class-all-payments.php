...
use WC_FatoriPay\Meta_Data\Subscription_Meta;
use WC_FatoriPay\Billing_Type\Ticket as Ticket_Type;
use WC_FatoriPay\Api\Response\Error_Response;
use WC_FatoriPay\WC_FatoriPay;
use WC_FatoriPay\Helper\Validation_Helper;
use WC_FatoriPay\Installments\Admin\Settings\Installments_Settings;
use WC_Order;
use wpdb;

/**
 * FatoriPay ticket gateway
 */
class All_Payments extends Gateway {
	/**
	 * Init the gateway
	 */
	public function __construct() {
		...
		$this->method_title = __( 'FatoriPay Gateway', 'fatoripay-woo' );
		/* translators: %s: FatoriPay website URL  */
		$this->method_description = sprintf( __( 'Use <a href="%s">FatoriPay</a> para permitir pagamentos através do nosso redirecionamento.', 'fatoripay-woo' ), 'https://fatoripay.com.br/' );
	...
	public function append_html_to_thankyou_page( $order_id ) {
		...
		if ( $this->id === $order->get_wc()->get_payment_method() ) {
			WC_FatoriPay::get_instance()->get_template_file( 'order/ticket-thankyou.php', array( 'order' => $order ) );
		}
	}
	 ...
	 *
	 * Check in FatoriPay the minimum due date for the ticket. If the validity date is less than the minimum, use the
	 * minimum as due date.
	 *
	...
	public function create_due_date( $reference_date = 'now', $ignore_validity_days = false ) {
		...
		if ( false === $ignore_validity_days ) {
			$validity_days = apply_filters( 'woocommerce_fatoripay_default_ticket_validity_days', 3 );
	...
	/**
	 * Process a ticket order to FatoriPay API
	 *
	...
	public function process_payment( $order_id ) {
		...
		if ( $meta ) {
			...
			$this->fatoripay_customer_id = $meta->customer;
		}
		...
		$payment_data = array(
			'customer'          => false === is_null( $this->fatoripay_customer_id ) ? $this->fatoripay_customer_id : $customer_meta['id'],
			'billingType'       => $this->type->get_id(),
			'value'             => $total,
			'dueDate'           => $this->create_due_date()->format( 'Y-m-d' ),
			'externalReference' => $id,
			/* translators: %d: the order id  */
			'description'       => sprintf( __( 'Order #%d', 'fatoripay-woo' ), $id ),
		);
		...
		foreach ( $transactions_queue as $transaction ) {
			switch ( $transaction['type'] ) {
				...
				case 'single':
					...
					$response = apply_filters( 'woocommerce_fatoripay_process_payment_api_response', $this->api->payments()->create( $transaction['payment_data'] ), $transaction['payment_data'] );
				...
				case 'subscription':
					/* @var Response $response The API response. */
					$response = apply_filters( 'woocommerce_fatoripay_process_subscription_api_response', $this->api->subscriptions()->create( $transaction['payment_data'] ), $transaction['payment_data'] );
					...
					if ( 0 !== $transaction['first_payment_strategy']['processed_by_parent_order'] && false === $order->get_meta_data() ) {
						...
						if ( 0 < $response->get_json()->totalCount ) {
							...
							$this->send_checkout_failure(
								array(
									__( 'There was a failure on subscription generation.', 'fatoripay-woo' ),
								)
							);
						}
					}
	...
	private function get_payment_fields() {
		...
		return apply_filters(
			'woocommerce_fatoripay_ticket_payment_fields',
			$fields,
			$this
		);
	}
	...
	public function get_posted_data() {
		...
		foreach ( $payment_fields as $key => $field ) {
			$field_type = WC_FatoriPay::get_instance()->get_form_field_object_from_type( $field['type'] );
		...
		return apply_filters( 'woocommerce_fatoripay_ticket_posted_data', $data );
	}
	...
	/**
	 * Removes the FatoriPay platform ticket after the ticket expires.
	 *
	...
	public function remove_expired_ticket() {
		...
		$validity_period = apply_filters( 'woocommerce_fatoripay_default_ticket_validity_period', '' );
		if ( isset( $this->settings['validity_period'] ) && '' !== $this->settings['validity_period'] ) {
			$validity_period = intval( $this->settings['validity_period'] );
		}

		if ( '' === $validity_period ) {
			return esc_html__( 'Ignore remove tickets by empty setting.', 'fatoripay-woo' );
		}
		...
		if ( 200 !== $payments->code || 0 === count( $payments->items ) ) {
			return esc_html__( 'No expired ticket to remove.', 'fatoripay-woo' );
		}
		...
		foreach ( $payments->items as $payment ) {
			$order_meta_count       = intval(
				$wpdb->get_var(
					$wpdb->prepare(
						"SELECT COUNT(*) FROM $wpdb->postmeta WHERE meta_key = '_fatoripay_id' AND meta_value = %s", array(
							'_fatoripay_id' => $payment->id,
						)
					)
				)
			);
		...
		return sprintf( __( 'Total tickets removed: %d', 'fatoripay-woo' ), $totals );
	}
