...
namespace WC_FatoriPay\Gateway;

use WC_FatoriPay\Api\Api;
use WC_FatoriPay\WC_FatoriPay;
use WC_FatoriPay\Billing_Type\Billing_Type;
use WC_FatoriPay\Admin\Settings\Settings;
use WC_FatoriPay\Log\Logger;
use WC_FatoriPay\Helper\WP_Error_Helper;
use WC_FatoriPay\Meta_Data\Customer;
use WC_FatoriPay\Api\Response;
use WC_FatoriPay\Api\Response\Error_Response;
use WC_FatoriPay\Log\Logger_Legacy;
use WC_FatoriPay\Meta_Data\Order;
...
abstract class Gateway extends \WC_Payment_Gateway {
	 ...
	 * @var string
	 */
	const PREFIX_LOG = 'FatoriPay: ';

	/**
	 * FatoriPay API wrapper.
	 *
	 * @var Api
	 */
	protected $api;
	...
	/**
	 * The FatoriPay customer id used to process the payment
	 *
	 * @var string
	 */
	protected $fatoripay_customer_id;
	...
	/**
	 * Get FatoriPay customer metadata object
	 *
	...
	public function process_admin_options() {
		...
		$gateways = WC_FatoriPay::get_instance()->get_gateways();
	...
	/**
	 * Integrate the order with FatoriPay
	 *
	 * Create or update an FatoriPay customer to process the order. If the API returns some error, the process will be
	...
	public function set_customer( $customer_id, $data ) {
		...
		$params = apply_filters( 'woocommerce_fatoripay_set_customer_params', $params, $customer, $action );
		$action = apply_filters( 'woocommerce_fatoripay_set_customer_action', $action, $customer, $params );

		/* @var Response $response The API response. */
		$response = apply_filters( 'woocommerce_fatoripay_set_customer_api_response', call_user_func_array( array( $this->api->customers(), $action ), $params ) );
		...
		$this->fatoripay_customer_id = $response->id;
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( false === $order ) {
			return new \WP_Error( 'error', __( 'Order not found.', 'fatoripay-woo' ) );
		}

		if ( ! $this->can_refund_order( $order ) ) {
			return new \WP_Error( 'error', __( 'This order cannot be refunded via this flow. Please, refund through your payment gateway or externally.', 'fatoripay-woo' ) );
		}

		if ( $amount < $order->get_total() ) {
			return new \WP_Error( 'error', __( 'Partial refunds are not possible.', 'fatoripay-woo' ) );
		}

		if ( 'fatoripay-ticket' === $this->id ) {
			return new \WP_Error( 'error', __( 'Due to payment method, to return funds to the customer you will need to issue a refund through your payment gateway or externally.', 'fatoripay-woo' ) );
		}

		$payment_id = $this->get_payment_id_from_order( $order );
		if ( false !== $payment_id ) {
			/* @var Response $response The API response. */
			$response = apply_filters( 'woocommerce_fatoripay_process_payment_refund_api_response', $this->api->payments()->refund( $payment_id ), $this );
			if ( is_a( $response, Error_Response::class ) ) {
				return $response->get_errors();
			}

			/* translators: %s: FatoriPay payment id  */
			$note = sprintf( __( '%s payment refund request has been processed successfully.', 'fatoripay-woo' ), $payment_id );
			$order->add_order_note( self::PREFIX_LOG . ' ' . $note );
		} else {
			return new \WP_Error( 'error', __( 'Payment not found. The order cannot be refunded via this flow.', 'fatoripay-woo' ) );
		}
				...
				if ( in_array( $response->get_json()->status, $refundable_statuses, true ) ) {
					$refund = wc_create_refund(
						array(
							...
							'reason'         => __( 'Order automatically refunded due to total or partial processing failure.', 'fatoripay-woo' ),
							'refund_payment' => true,
						)
					);
	...
	protected function generate_transactions_queue( $gateway, $order_id, $payment_data ) {
		...
		if ( true === $order->has_subscription() ) {
			...
			foreach ( $subscriptions as $subscription ) {
				...
				$subscription_payment_data['description'] = sprintf( __( 'Subscription #%1$d - %2$s', 'fatoripay-woo' ), $subscription->get_id(), implode( ', ', $items_names ) );

				$subscription_payment_data['externalReference'] = $subscription->get_id();
				$subscription_payment_data                      = apply_filters( 'woocommerce_fatoripay_subscription_payment_data', $subscription_payment_data, $wc_order, $subscription, $subscription_item, $gateway );
				$transactions_queue[]                           = array(
					'type'                   => 'subscription',
			...
			if ( true === $need_single_payment_transaction && $total > 0 ) {
				...
				if ( 'fatoripay-pix' !== $this->id ) {
					$payment_data = apply_filters( 'woocommerce_fatoripay_payment_data', $payment_data, $wc_order, $gateway );
				}
		...
		return apply_filters( 'woocommerce_fatoripay_transactions_queue', $transactions_queue, $wc_order, $gateway );
	}

	/**
	 * Adds FatoriPay payment id to order meta
	 *
	 * @param string   $payment_id FatoriPay payment id.
	 * @param WC_Order $order The checkout order.
	 * @return void
	 */
	public function add_payment_id_to_order( $payment_id, $order ) {
		$order->add_meta_data( '_fatoripay_id', $payment_id );
		$order->save();
	}
	...
	/**
	 * Gets FatoriPay payment id from order meta
	 *
	 * @param WC_Order $order The checkout order.
	 * @return string|false
	 */
	public function get_payment_id_from_order( $order ) {
		$payment_id = $order->get_meta( '_fatoripay_id', true );
