<?php
/**
 * Abstract user for FatoriPay features
 *
...
/**
 * Abastract user for FatoriPay features.
 */
class Customer {
	 ...
	 * @var string
	 */
	const META_KEY = 'wocommerce-fatoripay-customer';
	 ...
	 *
	 * @param string $key The FatoriPay customer meta key.
	...
	public function set_meta( $key, $value ) {
		if ( ! in_array( $key, $this->available_meta, true ) ) {
			...
			return new \WP_Error( sprintf( __( 'Invalid customer meta key. The available keys are: %s', 'fatoripay-woo' ), implode( ', ', $this->available_meta ) ) );
		}
	...
	/**
	 * Extract required FatoriPay data from order meta data.
	 *
	 ...
	 * @return array The customer data fomatted to FatoriPay API.
	 */
	...
	/**
	 * Get all FatoriPay customers
	 *
	 * FatoriPay customers are the WP users that has this class META_KEY in your metadata.
	 *
	 * @return self[] All FatoriPay customers.
	 */
