...
class Subscription_Admin {
	...
	public function send_validation_error( $error_message ) {
		wp_die(
			'<b>' . esc_html__( 'Subscription update error:', 'fatoripay-woo' ) . '</b> ' . esc_html( $error_message ),
			esc_html__( 'Subscription update error', 'fatoripay-woo' ),
	...
	public function validate_subscription_changes( $post_ID, $post_data ) {
		...
		// Is not using FatoriPay payment gateway?
		$payment_gateway = wc_get_payment_gateway_by_order( $subscription );
		if ( false === $payment_gateway || false === strpos( $payment_gateway->id, 'fatoripay-' ) ) {
			return;
		}
		...
		if ( $this->date_changed( $subscription->get_time( 'start' ), $start_timestamp ) ) {
			$this->send_validation_error( __( 'FatoriPay does not support changes on subscription start date.', 'fatoripay-woo' ) );
			return;
		}
		if ( $this->date_changed( $subscription->get_time( 'trial_end' ), $trial_end_timestamp ) ) {
			$this->send_validation_error( __( 'FatoriPay does not support changes on trial date.', 'fatoripay-woo' ) );
			return;
		}

		$now = date( 'Y-m-d' );
		if ( date( 'Y-m-d', $next_payment_timestamp ) <= $now ) {
			$this->send_validation_error( __( 'The next payment date must be greater than the current one.', 'fatoripay-woo' ) );
			return;
		}
		if ( '' !== $end_date_timestamp && '0' !== $end_date_timestamp && date( 'Y-m-d', $end_date_timestamp ) <= $now ) {
			$this->send_validation_error( __( 'The end subscription date must be greater than the current one.', 'fatoripay-woo' ) );
			return;
		}
		...
		if ( false === $new_billing_cycle ) {
			$this->send_validation_error(
				...
				sprintf( __( 'FatoriPay does not support the chosen billing period. Please, choose a valid period: %s.', 'fatoripay-woo' ), $subscriptions_helper->get_supported_billing_periods_string() )
			);
			return;
		}

		// Needs tokenization and is it unavailable?
		if ( 'fatoripay-credit-card' === $payment_gateway->id ) {
			if ( $new_billing_cycle !== $previous_billing_cycle ||
				...
				if ( false === $tokenization_available ) {
					$this->send_validation_error(
						__( 'To update the billing cycle, next payment date or subscription end date you need to have tokenization enabled in your FatoriPay account.', 'fatoripay-woo' ) .
						__( 'Please, contact your FatoriPay manager.' )
					);
					return;
				}
			}
		...
		// Updates the FatoriPay subscription.
