...
namespace WC_FatoriPay\Subscription;
...
use WC_FatoriPay\Webhook\Webhook;
...
class Subscription {
	...
	/**
	 * The FatoriPay subscription statuses
	 *
	 * @var string
	 */
	...
	/**
	 * WoocCommerc x FatoriPay subscription statuses
	 *
	 * @var array
	 */
	protected $woo_x_fatoripay_subscription_statuses = array();
	...
	private function __construct() {
		$this->woo_x_fatoripay_subscription_statuses = array(
	...
	/**
	 * Syncronizes WooCommerce subscription status with FatoriPay subscription status
	 *
	...
	public function sync_status( $order_id, $status_from, $status_to, $subscription ) {
		...
		if ( $payment_gateway && array_key_exists( $status_to, $this->woo_x_fatoripay_subscription_statuses ) ) {
			...
			if ( $subscription_id ) {
				$new_status = $this->woo_x_fatoripay_subscription_statuses[ $status_to ];
				$payload    = array(
					'status' => $this->woo_x_fatoripay_subscription_statuses[ $status_to ],
				);
