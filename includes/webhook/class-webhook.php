...
class Webhook {
	...
	public function process_event() {
		switch ( $this->event ) {
			...
			default:
				...
				die( esc_html( sprintf( __( 'Untreated event: %s', 'fatoripay-woo' ), $this->event ) ) );
		}
	}
	...
	private function on_payment_confirmed() {
		...
		$this->add_order_note( __( 'Payment confirmed.', 'fatoripay-woo' ) );
	}
	...
	private function process_payment() {
		...
		if ( $this->order->has_status( $paid_statuses ) ) {
			throw new Inconsistency_Data_Exception( esc_html__( 'This order was already paid.', 'fatoripay-woo' ) );
		}
	...
	private function on_payment_created() {
		if ( isset( $this->data->payment->subscription ) ) {
			...
			if ( 200 !== $response->code ) {
				throw new \Exception( sprintf( 'Error getting payments for a subscription in FatoriPay. Response HTTP status: %d', $response->code ) );
			}
			...
			if ( false === $order_meta->get_meta_data() ) {
				if ( 'fatoripay-credit-card' === $this->gateway->id ) {
					$order_meta->set_meta_data( $this->data->payment );
				} elseif ( 'fatoripay-ticket' === $this->gateway->id ) {
					...
					$order_meta->set_meta_data( $this->data->payment );
				} elseif ( 'fatoripay-pix' === $this->gateway->id ) {
					...
					if ( is_a( $pix_info_response, Error_Response::class ) ) {
						...
						throw new \Exception( sprintf( 'Error getting PIX information for a payment subscription in FatoriPay. Response HTTP status: %d', $pix_info_response->code ) );
					}
			...
			// Tries to set the new externalReference for payment in FatoriPay (this is only possible if the payment is still pending).
			$payment_data = array(
			...
			$note = sprintf( __( 'Payment created. Due date: %1$s / Subscription id: %2$s', 'fatoripay-woo' ), $due_date, $this->subscription->get_id() );
			$transaction_order->add_order_note( self::PREFIX_LOG . ' ' . $note );
		} else {
			$this->order->update_status( 'pending' );
			$this->add_order_note( __( 'Payment created.', 'fatoripay-woo' ) );
		}

		if ( ! isset( $this->data->payment->creditCard ) && isset( $this->data->payment->invoiceNumber ) ) {
			$payment_url    = sprintf( 'https://%1$s.fatoripay.com/payment/show/%2$s', ( false === strpos( $this->data->payment->invoiceUrl, 'sandbox' ) ? 'www' : 'sandbox' ), $this->data->payment->invoiceNumber );
			$payment_anchor = sprintf( '<a href="%1$s" target="_blank">%1$s</a>', $payment_url );

			/* translators: 1: The payment link  */
			$note = sprintf( __( 'Payment link: %1$s', 'fatoripay-woo' ), $payment_anchor );
	...
	private function on_payment_deleted() {
		...
		$this->add_order_note( __( 'Payment deleted.', 'fatoripay-woo' ) );
	}
	...
	private function on_payment_overdue() {
		...
		$this->add_order_note( __( 'Payment overdue.', 'fatoripay-woo' ) );
	}
	...
	private function update_order_values_on_credit_card() {
		if ( $this->payment->value > $this->payment->originalValue ) {
			...
			if ( isset( $this->payment->fine->value ) ) {
				...
				$item_name = __( 'Fine tax', 'fatoripay-woo' );
			...
			if ( isset( $this->payment->interest->value ) ) {
				...
				$item_name = __( 'Interest', 'fatoripay-woo' );
				$this->add_item_fee_to_order( $item_name, $item_total );
			}
		}
	...
	private function on_payment_refunded() {
		...
		$this->add_order_note( 'Payment refunded.', 'fatoripay-woo' );
	}
	...
	private function on_payment_restored() {
		...
		$this->order->update_status( 'pending' );
		$this->add_order_note( 'Payment restored.', 'fatoripay-woo' );
	}
	...
	private function on_payment_updated() {
		...
		$this->add_order_note( 'Payment updated', 'fatoripay-woo' );
	}
