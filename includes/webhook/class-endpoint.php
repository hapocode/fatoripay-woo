...
class Endpoint {
	 ...
	 * @var string
	 */
	private $query_var = 'fatoripay-webhook';
	 ...
	 * @var string
	 */
	private $url_endpoint = 'fatoripay-webhook';
	...
	/**
	 * Add rewrite rule for fatoripay-webhook
	 */
	...
	public function process_webhook() {
		if ( '1' === get_query_var( $this->query_var ) ) {
			try {
				...
				$this->gateway = WC_FatoriPay::get_instance()->get_gateway_by_billing_type( $data->payment->billingType );
				...
				$this->response( 200, __( 'Webhook has been processed with success', 'fatoripay-woo' ) );
	...
	private function validate_event( $event ) {
		...
		if ( false === array_search( $event, $accepted_events, true ) ) {
			...
			throw new Event_Exception( sprintf( __( 'Event %s wasn\'t registered.', 'fatoripay-woo' ), $event ) );
		}
	}
	...
	private function validate_status( $data ) {
		...
		if ( 200 !== $response->code ) {
			throw new \Exception( sprintf( 'Error verifying payment status in FatoriPay. Response HTTP status: %d', $response->code ) );
		}
		...
		if ( $data->payment->status !== $response->get_json()->status ) {
			throw new Inconsistency_Data_Exception( 'Status doesn\'t match with FatoriPay' );
		}
	}
